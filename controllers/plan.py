...
class MainHandler(webapp2.RequestHandler):
    def get(self):
        ...
        for semester in semesters:
            ...
            for course_id in semester['courses']:
                ...
                for prereq in course['prerequisites']:
                    prereqs.append(models.course.get_course(prereq).to_json())
                course['prerequisites'] = prereqs
                coreqs = []
                for coreq in course['corequisites']:
                    coreqs.append(models.course.get_course(coreq).to_json())
        ...
        unmet_prereqs = rules.rules.evaluate_prerequisites(my_courses)
        logging.info('Unmet Prereqs: %s', unmet_prereqs)
        unmet_coreqs = rules.rules.evaluate_corequisites(my_courses)
        logging.info('Unmet Coreqs: %s', unmet_coreqs)

        messages = []
        for course in unmet_prereqs:
            messages.append(course['subject_code'] + ' ' + course['number'] + ' is missing one or more of ' +
                                                                              'its prerequisites')
        for course in unmet_coreqs:
            messages.append(course['subject_code'] + ' ' + course['number'] + ' is missing one or more of ' +
                                                                              'its corequisites')
